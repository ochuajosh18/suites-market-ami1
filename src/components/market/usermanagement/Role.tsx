...
class Role extends React.PureComponent<RoleProps> {
            ...
            headerEndButton:
                <div>
                    <AddRoleButton 
                        id="add-role-btn"
                        startIcon={<AddIcon />}
                        onClick={this._onToggleModal.bind(this, true)}
                    >
                        Add New
                    </AddRoleButton>
                </div>
    ...
    _onToggleModal = (open: boolean) => {
        (open) 
            ? this.props.setUserManagementState({ modalVisible: open as boolean})
            : this.props.setUserManagementState({ 
                modalVisible: open as boolean, 
                selectedRoleId: "",
                selectedRoleName: "",
    ...
    _onDeleteClick = async () => {
        await this.props.deleteRole(this.props.usermanagement.selectedRoleId);
    }
    ...
    _onSaveClick = async () => {
        ...
        if(selectedRoleName && selectedRoleDescription && roleIndex === -1) {
            await this.props.saveRole(selectedRoleId, selectedRoleName, selectedRoleDescription)
        }
    }
