...
describe('Market Order History List', () => {
    let store: ReturnType<typeof createTestStore>;

    it('Should show the card for a historical order', async () => {
    ...
    it('Should be able to use the tabs as a filter', () => {
        store = createTestStore();
        const wrapper = setup(store); 
        wrapper.find('#order-history-cancelled-tab').at(0).simulate('click');
        wrapper.update();
        expect(store.getState().ordermanagement.orderHistoryTab).toBe('Cancelled');
    });
});